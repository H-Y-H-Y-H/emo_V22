...
if __name__ == "__main__":
    ...
    save_pth = '../../dataset1109/'
    os.makedirs(save_pth,exist_ok=True)
    os.makedirs(save_pth+'img/',exist_ok=True)


    load_cmd = np.loadtxt('../data/nvidia/mimic_synced_cmds(rank1).csv')

    lmks = np.load('../data/nvidia/m_lmks_synthesized.npy')
    ...
    min_dist = +np.inf
    for j in range(8):
        cmd_motor = np.clip(load_cmd[0][j],0,1)
        all_motors[j].norm_act(cmd_motor)
    time.sleep(1)
    print('start')
    num_motors = 8
    step_num = 4
    with mp_face_mesh.FaceMesh(
        ...
        for idx in range(len(load_cmd)):
            if img_i == 0:
                for _ in range(100):
                    ret, image = cap.read()

            print('Target Face',idx)
            ref_cmds = load_cmd[idx]
            pout_lmks = lmks[idx]
            pout_lmks_mouth = pout_lmks[mouth_lmks]
            min_dist = +np.inf
            for kk in range(100):
                target_cmds = np.random.normal(ref_cmds,scale = 0.1)[:num_motors]
                target_cmds = np.clip(target_cmds,0,1)

                target_cmds[4] = target_cmds[3]
                target_cmds[6] = target_cmds[5]
                ...
                for i_step in range(step_num):
                    ...
                    action_logger.append(traj[i_step])

                    ret, image = cap.read()
                    image = image[:,(640-480)//2:(640+480)//2 ]

                    # image = image[(480-320)//2:(480+320)//2,(640-320)//2:(640+320)//2 ]

                    image_show, raw_lmks, m_lmks = render_img(image, face_mesh, pcf)

                    mouth_m_lmks = m_lmks[mouth_lmks]

                    m_lmks_logger.append(m_lmks)
                    r_lmks_logger.append(raw_lmks)

                    # SAVE
                    cv2.imwrite(save_pth+'/img/%d.png' % img_i, image_show)

                    if img_i % 20 == 0:
                        np.save(save_pth+'/m_lmks.npy', np.asarray(m_lmks_logger))
                        np.save(save_pth+'/r_lmks.npy', np.asarray(r_lmks_logger))
                        np.savetxt(save_pth+'/action.csv',action_logger)
                        np.savetxt(save_pth+'/cmds_idx.csv',best_id_logger)

                    img_i += 1 
                    dist = np.sum((mouth_m_lmks - pout_lmks_mouth)**2)
                    if dist<min_dist:
                        print('new:', dist)
                        best_id_logger.append(img_i)
                        min_dist = dist
                        ref_cmds = np.copy(traj[i_step])
