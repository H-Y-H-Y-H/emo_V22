...
if __name__ == "__main__":
    ...
    # move commands list slowly and record it.
    elif mode == 4:
        import sys
        sys.path.append('../../test/')
        from camera_test import *
        ...
        idx_cmds = 11
        model_name = 'basic_action'

        # load_cmd = np.loadtxt('action_source.csv')
        load_cmd = np.hstack((load_cmd[:,:4],load_cmd[:,3:4],load_cmd[:,4:5],load_cmd[:,4:5],load_cmd[:,5:6]))
        ...
        for i in range(len(load_cmd)):
            ...
            move_all(target_cmds, with_eyes=False)
            time.sleep(1)
        recorder.stop_recording()
    ...
    # data_collect only pictures
        ...
        ref_list = np.loadtxt('action_source.csv')
        ...
        while 1:
            ...
            for i in range(step_num):
                ...
                # SAVE
                cv2.imwrite(save_data_pth+'img/%d.png' % img_i, image)
                img_i += 1
                if img_i % 1000 == 0:
